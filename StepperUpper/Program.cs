...
namespace StepperUpper
{
    internal static class Program
    {
        ...
        private static Task CleanDawnguardAsync(Merged m)
        {
            ...
            // Dawnguard has a special manual step.
            Action<Record> beforeFinalize = rec => Doer.DeleteField(rec, 0x16BCF, B4S.XEZN);

            return CleanDlcAsync("Dawnguard", deletes, udrs, m, beforeFinalize);
        }
        ...
        private static async Task CleanDlcAsync(string dlcName, HashSet<uint> deletes, uint[] udrs, Merged m, Action<Record> beforeFinalize = null)
        {
            deletes.UnionWith(udrs);
            ...
            beforeFinalize?.Invoke(dlcMaster);
